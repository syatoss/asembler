...
void push_next_string(StrArr* array, char* str) {
    ...
    str_cp = (char*)malloc(sizeof(char) * strlen(str));
    if(!str_cp) SYS_MEM_FAIL_EXIT(1);
    new_string_local =
        array->strings != NULL?
       (char**)realloc(array->strings, sizeof(char*) * (array->length + 1))
...
char* get_str_at_index(StrArr* array, int index) {
    if(index >= array->length) {
        fprintf(stderr, "Invalid use of function \"get_str_at_index\" index exceeds the array length");
        exit(2);
    }
...
StrArr* split(char* str, char* delimiter) {
    ...
    char* token;
    char* pivot;
    char* str_cp = (char*)malloc(sizeof(char) * strlen(str));
    str_cp = strcpy(str_cp, str);
    pivot = str_cp;
    str_cp = trim(str_cp);
    arr = new_str_arr();
