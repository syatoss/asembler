...
int translationNeedsCompletion(Translation* trans) {
    ...
    for (i = 0; i < MAX_WORDS_PER_INSTRUCTION; i++)
        if (trans->nulls[i] != NULL) return true;
    return false;
}
...
void addMissingLabelAdresses(Translation* trans) {
    ...
    Label* currentLabel;
    for (i = 0; i < MAX_WORDS_PER_INSTRUCTION; i++) {
        if (trans->nulls[i] == NULL) continue;
        currentLabel = getLabelByName(ds->lable_tb, trans->nulls[i]);
        if (!currentLabel) continue;
        trans->binary[i] = intToBinary(currentLabel->lineOfApearance +
                                       ds->instructions_tb->translationCounter +
                                       START_ADDRESS);
...
void writeLabelApearanceLineToFile(Label* label, FILE* targetFile) {
    ...
    padding +=
        label->type == DATA ? ds->instructions_tb->translationCounter : 0;
...
void writeAsmTranslationTableToFile(AsmTranslationTable* table,
    ...
    while (currentRow) {
        writeRowTranslationToFile(currentRow->data, targetFile,
                                  startingAddress);
        currentRow = currentRow->next;
    }
}
...
void writeAsmOutput(AsmDescriptor* ds) {
    if (ds->lable_tb->hasEntryLabels) writeEntryFile(ds);
    if (ds->lable_tb->hasExternalLabels) writeExtrenalsFile(ds);
